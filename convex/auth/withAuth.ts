...
export function queryWithAuth<
  ...
  Output
>({ args, handler }: QueryWithAuth<ArgsValidator, Output>) {
  return query({
    args: {
      ...args,
    ...
    handler: async (ctx, args: any) => {
      ...
      const userSessionContext = await getValidExistingSession(
        auth,
        args.sessionId
      );
      return handler(
...
export function mutationWithAuth<
  ArgsValidator extends PropertyValidators,
  Output
>({ args, handler }: MutationWithAuth<ArgsValidator, Output>) {
  return mutation({
    args: { ...args, sessionId: v.union(v.null(), v.string()) },
    handler: async (ctx, args: any) => {
